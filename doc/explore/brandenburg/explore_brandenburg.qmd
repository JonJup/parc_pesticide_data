---
title: "explore brandenburg pesticide data"
...
```{r}
sliderInput("bins", "Binwidth:", 
            min = 1, max = 50, value = 30)
plotOutput("distPlot")
```

```{r}
data = readRDS("../../../data/bandenburg/pesticide_data_brandenburg_clean.rds")
```
...
unique(data, by = "compound") |> dplyr::arrange(desc(n.occur)) |> slice_head(n = 20) |> ggplot(aes(n.occur, reorder(compound, n.occur, sum))) + geom_col()
```
...
```{r}
sites.sf <- st_as_sf(sites, coords = c("x.coord", "y.coord"), crs = "EPSG:25833")
